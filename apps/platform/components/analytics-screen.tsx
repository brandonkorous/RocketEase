    ...
    <div className="mx-auto flex w-full max-w-360 flex-col gap-4 px-4 py-5 lg:px-8">
      ...
      <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
        ...
        <section className="rounded-box border border-base-300 p-4 xl:col-span-2" aria-label="Reports">
          <div className="flex items-center justify-between"><h2 className="text-sm font-semibold">Reports</h2><span className="flex items-center gap-3"><Link href={workspacePath(data.workspaceId, "analytics/listening")} className="text-xs font-medium hover:underline">Listening →</Link><Link href={workspacePath(data.workspaceId, "analytics/recommendations")} className="text-xs font-medium hover:underline">Recommendations →</Link><Link href={workspacePath(data.workspaceId, "reports")} className="text-xs font-medium hover:underline">View all reports →</Link></span></div>
