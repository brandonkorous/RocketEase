...
/** `settings`: the network's own knobs for this variant (a Business Profile post's type, button, event or offer). */
export type VariantState = { format: string; textOverride: string | null; assetIdsOverride: string[] | null; firstComment: string | null; linkOverride: string | null; settings: Record<string, unknown>; validation: ValidationIssue[] };
...
export type Override = { textOverride: string | null; firstComment: string; linkOverride: string | null; settings: Record<string, unknown> };
export const EMPTY_OVERRIDE: Override = { textOverride: null, firstComment: "", linkOverride: null, settings: {} };
