...
import { fmtBytes, fmtDur, type AssetCard, type LibraryData } from "./types";
    ...
    <div className="min-w-0 p-4" onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); if (data.canEdit) onDropFiles(e.dataTransfer.files); }}>
      ...
      {data.assets.length === 0 ? (
        ...
        <ul className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
          {data.assets.map((a) => (<AssetTile key={a.id} a={a} selected={a.id === data.selected?.id} checked={checked.has(a.id)} onOpen={() => nav({ asset: a.id, page: String(data.page) })} onCheck={(on) => { const n = new Set(checked); on ? n.add(a.id) : n.delete(a.id); setChecked(n); }} />))}
        </ul>
      )}
...
function AssetTile({ a, selected, checked, onOpen, onCheck }: { a: AssetCard; selected: boolean; checked: boolean; onOpen: () => void; onCheck: (on: boolean) => void }) {
  return (
    <li className={`overflow-hidden rounded-lg border ${selected ? "border-base-content" : "border-base-300"}`}>
