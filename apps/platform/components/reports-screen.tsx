...
export type ReportsData = { workspaceId: string; definitions: ReportListRow[]; runs: RunRow[]; channels: { id: string; name: string }[]; external: ExternalRecipientRow[]; canManage: boolean; pdfAvailable: boolean; newInitial: ReportFormInitial | null };
  ...
  return (
    <div className="mx-auto flex w-full max-w-360 flex-col gap-4 px-4 py-5 lg:px-8">
      ...
      <section className="rounded-box border border-base-300 p-4" aria-label="Report history">
        ...
        {!data.pdfAvailable && data.runs.some((r) => r.format === "HTML") && (
          <p className="mt-1 text-xs text-secondary/70">Branded documents download as HTML. PDF arrives when a Chromium renderer is configured for this deployment — nothing here is converted or faked.</p>
        )}
        <table className="mt-3 w-full text-sm">
