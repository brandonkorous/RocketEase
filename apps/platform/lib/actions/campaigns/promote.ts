...
export async function promoteVariant(workspaceId: string, input: PromoteInput): Promise<ActionState & { promotionId?: string; stepUpRequired?: true }> {
  return guard(async () => {
    ...
    if (!["healthy", "degraded"].includes(ch.status)) return fail("Reconnect this channel before promoting.");
    if (!ch.capabilities.ads.manage) return fail(ch.capabilities.reasons?.ads ?? `${ch.name} does not allow promotions from Make It Social.`);
    if (!account || !conn || conn.status !== "active") return fail("Pick a connected ad account.");
    if (account.provider !== ch.provider) return fail("The ad account must belong to the same network as the post.");
    if (!getAdapter(conn.provider).promote) return fail("This network only supports read-only ad import.");
    if (!OBJECTIVES.includes(input.objective)) return fail("Pick an objective.");
    const amount = Number(input.amount);
    if (!Number.isFinite(amount) || amount <= 0 || amount > MAX_AMOUNT) return fail(`Budget must be between 0 and ${MAX_AMOUNT.toLocaleString()} ${account.currency}.`);
    const startAt = input.startAt ? zonedToUtc(input.startAt, ctx.workspace.timezone) : new Date();
    const endAt = input.endAt ? zonedToUtc(input.endAt, ctx.workspace.timezone) : null;
    if (endAt && endAt <= startAt) return fail("End must be after start.");
    if (input.budgetKind === "lifetime" && !endAt) return fail("A lifetime budget needs an end date.");
    const days = endAt ? Math.max(1, Math.ceil((endAt.getTime() - startAt.getTime()) / DAY)) : 7;
    const est = input.budgetKind === "daily" ? amount * days : amount;
    ...
    const request: Omit<PromotionRequest, "idempotencyKey"> = { name: input.name.trim() || `${item.title} · boost`, objective: input.objective, sourcePostRemoteId: variant.remoteId, channelRemoteId: ch.remoteId, budget: { kind: input.budgetKind, amount, currency: account.currency }, startAt: startAt.toISOString(), endAt: endAt?.toISOString(), audience: countries.length ? { countries } : undefined, link: variant.linkOverride ?? item.link ?? undefined, tracking: campaign ? { utmSource: campaign.tracking.utmSource, utmMedium: campaign.tracking.utmMedium, utmCampaign: campaign.tracking.utmCampaign } : undefined, initialStatus: input.initialStatus === "active" ? "active" : "paused" };
