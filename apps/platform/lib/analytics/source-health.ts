...
const OURS = /no longer reports/i;
...
export function explainSyncError(lastError: string | null): SourceExplanation {
  ...
  if (e.includes("valid insights metric") || e.startsWith("validation")) {
    return { headline: "The network rejected part of what we asked for.", action: "Nothing to do — the rest still updates, and we drop retired metrics automatically." };
  }
