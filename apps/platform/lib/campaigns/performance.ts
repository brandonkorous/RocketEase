...
export type CampaignPerformance = { cards: CampaignCard[]; all: Totals; organic: Totals; paid: Totals; trend: SeriesPoint[]; top: TopPost[]; compareLabel: { from: string; to: string } | null; conversions: ConversionState; conversionProvenance: ConversionProvenance | null };
...
export async function campaignPerformance(workspaceId: string, campaignId: string, filters: AnalyticsFilters, cards: DisplayMetric[] = CAMPAIGN_CARDS): Promise<CampaignPerformance> {
  ...
  return { cards: cards.map((k) => card(k, input)), all, organic, paid, trend, top, compareLabel: cmp, conversions, conversionProvenance: conversionProvenance(conversions) };
}
