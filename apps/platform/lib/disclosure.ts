...
/**
 * The other direction: media a model made, with nothing declared about it.
 *
 * `disclosureGap` only fires once you HAVE declared synthetic media, so an author
 * who simply forgot published an unlabelled generated image and heard nothing.
 * The flag stays theirs to set — a declaration the product made on their behalf
 * is not a declaration — so this prompts rather than decides.
 */
export function undeclaredSyntheticGap(
  generatedFileNames: string[],
  sm: SyntheticMedia | null | undefined,
  opts: { required: boolean },
): DisclosureGap | null {
  if (generatedFileNames.length === 0) return null;
  if (toDisclosureInput(sm).synthetic) return null;
  const names = generatedFileNames.map((n) => `“${n}”`).join(", ");
  const what = generatedFileNames.length === 1 ? "was generated by AI" : "were generated by AI";
  const why = `${names} ${what}, but this post doesn't declare synthetic media.`;
  return opts.required
    ? { severity: "error", code: "synthetic_undeclared", message: `${why} This workspace requires disclosure.` }
    : { severity: "warning", code: "synthetic_undeclared", message: `${why} Set it under “Contains AI-generated media?” so networks can label it.` };
}
