...
export type ContactCard = { id: string; name: string; avatarUrl: string | null; handle: string | null; profileUrl: string | null; network: string; email: string | null; location: string | null; tags: string[]; since: string; moderations: number };
...
const EVENT_LABEL: Record<string, string> = {
  ...
  hide_requested: "Hide requested", hidden: "Comment hidden", hide_unsupported: "Could not hide — flagged", hide_failed: "Hide failed", hide_cancelled: "Hide cancelled", flagged: "Flagged", flag_cleared: "Flag cleared", unhide_requested: "Showing again requested", unhidden: "Comment shown again", unhide_failed: "Could not show again",
};
const activityLabel = (e: typeof conversationEvent.$inferSelect) => (e.kind === "automation" ? `Applied by rule ${String(e.data.ruleName ?? "")}`.trim() : (EVENT_LABEL[e.kind] ?? e.kind));

/** Names of the people who moderated anything in this thread, for "by". */
async function moderatorNames(msgs: Message[]) {
  const ids = [...new Set(msgs.map((m) => m.moderation?.actorUserId).filter((id): id is string => Boolean(id)))];
  const rows = ids.length ? await db.select({ id: user.id, name: user.name }).from(user).where(inArray(user.id, ids)) : [];
...
export async function conversationDetail(workspaceId: string, id: string, tz: string): Promise<ConversationDetailData | null> {
  ...
  const network = networkLabel(ch.network);
  const now = Date.now();
  return {
    ...
    activity: events.map(({ e }) => ({ id: e.id, label: activityLabel(e), at: fmt(e.createdAt), network: ch.network })),
