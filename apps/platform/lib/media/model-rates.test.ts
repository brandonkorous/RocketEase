 ...
 * (docs/bugs/B-013).
 *
 * Scoped to the adapters production actually configures — the azure ones. The
...
import { MODELS } from "@rocketease/media";
...
const routable = MODELS.filter((m) => m.adapter.startsWith("azure"));
...
describe("production media rates", () => {
  ...
  it.each(routable.map((m) => [m.key, m.cost.unit] as const))(
    ...
    (key) => {
      expect(Object.keys(rates("AI_MEDIA_RATES_JSON"))).toContain(key);
    },
  );
