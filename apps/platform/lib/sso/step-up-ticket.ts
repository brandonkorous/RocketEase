...
export type StepUpTicket = { nonce: string; purpose: StepUpPurpose; workspaceId: string; returnTo: string };
...
export function decodeTicket(raw: string | undefined): StepUpTicket | null {
  ...
  try {
    ...
    return { nonce: t.nonce, purpose: t.purpose, workspaceId: t.workspaceId, returnTo: safeReturnTo(t.returnTo) };
  } catch {
    return null;
  }
}
