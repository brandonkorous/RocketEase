...
| 14.5 | **Link-in-bio page** — Brand › Link in bio (a Brand tool, mockup approved 2026-09-05): one public page per workspace at `/l/:slug` (workspace slug, first free variant; hidden until Live, hidden = 404), ordered links saved as you go (URLs normalised, non-http refused, 20 max), a strip of the last 6 published posts with pictures from one channel, avatar from the kit's logo files, buttons black or brand primary with contrast-safe text; clicks are rows in `bio_link_click` written by the `/go` redirect (one click, one row; rolling 7 days + all time), never estimated. Custom domains and `conversion_fact` writes deferred. `docs/plans/m14.5-link-in-bio.md` | ✅ 2026-09-05 |
| 14.6 | **Comment moderation rules** — hide/flag/auto-reply-draft by rule (keyword, link, language, repeat offender) inside the inbox; every action is a `message`/audit row; hides call the provider's hide API where one exists and record "why not" where it does not. Built on the Settings › Automations engine (no second rules engine): new facts `has_link`, `link_hosts`, `language`, `prior_moderations`, `prior_messages`, operator `has_any` (whole-word keyword lists), actions `inbox.hide` / `inbox.flag` / `inbox.draft_reply`; `HIDE_SUPPORT` in `packages/providers` names the network document behind every hide (Facebook, Instagram, Threads, YouTube, X) and the reason for every network without one; a hide the network cannot do is recorded as a flag with that reason. Inbox: Flagged tab, Hide/Flag on a message, one status line per message, moderation count on the contact. `docs/plans/m14.6-comment-moderation.md` | ✅ 2026-09-05 |
| 14.7 | **DM rules engine** — model Meta's messaging windows and caps (24 h window, qualifying action, ~200 automated DMs/hr, 2026 one-automated-message rule); before a reply queues, say whether it can send and why not; surfaces on inbox reply and automations. Built: `MESSAGING_RULES` in `packages/providers` cites the document behind every network's DM rules (Meta's 24 h standard messaging window and 7-day Human Agent window, X's 15 per 15 min / 1,440 per day send caps) and the reason for every network without DMs; `sendDecision` (`lib/engagement/messaging`) is the ONE answer to "can this DM go out now?", read by the composer, the send actions, the rule action and the worker right before the network call. The "~200 automated DMs/hr" and the "2026 one automated message" rule appear in no Meta document (verified 2026-09-06), so neither is declared as Meta's: RocketEase applies its OWN rule of one automated DM per contact per 24 h on every network, and `message.rule_id` records which messages a rule wrote. `docs/plans/m14.7-dm-rules.md` | ✅ 2026-09-06 |
| 14.8 | **Google Business Profile posts** — the GBP adapter publishes `localPosts` (standard/offer/event) to a location; today it is reviews-only. Formats + limits from the Business Profile API; quota-0 gate already documented. Built: `posts.ts` publishes updates, events and offers with one photo and one button, reconciles an ambiguous create by scanning the location's newest posts, and reads `LIVE`/`PROCESSING`/`REJECTED` back (a rejected post is a policy failure, never a success). Every limit in `LIMITS` names its source; what Google does not publish (photos per post, video) is taken conservatively and said to be RocketEase's choice. Create gets a Business Profile panel under "Customize by platform" (type, button and link, event dates, offer details) stored on `post_variant.settings`; one validator (`postIssues`) serves the composer and the worker. Google's API was not called (quota 0). `docs/plans/m14.8-gbp-posts.md` | ✅ 2026-09-06 |
| 14.9 | **Canva / Adobe Express import** — resolve the M12 §12 open decision first (Canva Connect API vs Express embed); then "Import from Canva" into the library with rights = owned, provenance chain recorded. Decided: Canva Connect API now (it lists and renders a person's existing designs server-side), Adobe Express later (an editor embed, no listing/export API) on the same shape. Built: `import_source` (per person, per workspace; migration 0032), PKCE OAuth start/callback, a Canva client with every endpoint and limit sourced, locked single-use refresh, the `import.canva` job (one asset per page, in order; Canva's words on failure), and a library dialog (search, Previous/Next page, PNG or JPG). Rights = owned, `rights_note` names the design and account, `provenance.chain` records the design and export ids. Canva itself was not called: no integration credentials yet (launch chore). `docs/plans/m14.9-canva-import.md` | ✅ 2026-09-06 |
| 14.10 | **Per-client billing** — agencies bill each client workspace: client rate (M8.11) becomes an invoice line via Stripe Connect or Stripe Invoicing on the agency's own account; RocketEase never touches the money; statements in the agency overview. Built: Stripe Connect (Standard, OAuth) links the agency's own account (`agency_billing_account`, migration 0033); `buildStatement` turns the Economics row's inputs into invoice lines (fee, rebilled ad spend, rebilled AI) with one blocking reason when an input is unknown; Send creates the customer and invoice ON the agency's account (`Stripe-Account` header) and Stripe emails it; a Connect webhook marks paid/void only for the organization's own account; a Statements block in the agency overview shows status (○ Not sent / → Sent / ✓ Paid / ⊘ Void), Preview, Send, Open invoice, Refresh, Void, Connect/Disconnect; billing contact on the rate. Stripe itself was not called (no Connect client id yet). `docs/plans/m14.10-client-billing.md` | ✅ 2026-09-06 |
| 14.11 | **Listening + ad-library intelligence** — keyword/brand listening on networks whose APIs allow it (Bluesky, Threads keyword search, YouTube search — each as a connected account; X metered and Reddit approval-gated are off with reasons) and the EU DSA ad repositories (Meta Ad Library API, TikTok Commercial Content Library) as a competitor paid-creative feed; definitions and coverage gaps stated per network. `docs/plans/m14.11-listening.md` | ✅ |
| 14.12 | **Self-hosted agency tier** — licence-owned install: single-org mode (first sign-up claims it, then invitation-only via a Better Auth database hook), Ed25519 licence key checked offline, entitlements from the licence in Stripe's shape (30-day grace, then nothing new; no key = evaluation), Helm chart derived from `deploy/k8s`, `stable`/`edge` update channel with an opt-in feed check. `docs/plans/m14.12-self-hosted.md` | ✅ |
...
**The queue is Milestone 14, top to bottom** (user decision 2026-09-05: do all of it). 14.1–14.6 done 2026-09-05, 14.7–14.12 done 2026-09-06. Milestone 14 is complete; what remains is launch chores (credentials, reviews, the licence key ceremony, the first `v1.0.0` tag) listed in each `docs/plans/m14.N-*.md`.
