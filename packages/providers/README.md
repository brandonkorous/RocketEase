...
### Threads
- Product: the **Threads API** (`graph.threads.net/v1.0`), a separate use case with its **own app id and secret** on the Meta app; consent lives at `threads.net/oauth/authorize`. Env: `THREADS_APP_ID`, `THREADS_APP_SECRET`.
- Tokens: the code exchange returns a 1-hour token that the adapter immediately swaps for a 60-day one (`grant_type=th_exchange_token`); `refresh` calls `refresh_access_token` with `th_refresh_token`, which Threads allows only once the token is 24 hours old — the platform refreshes a day before expiry, so it always is. There is **no revocation endpoint** (documented no-op) and **no permissions endpoint**, so the recorded scopes are the requested ones.
- Scopes: `threads_basic`, `threads_content_publish`, `threads_read_replies`, `threads_manage_replies`, `threads_manage_insights`. Capabilities follow the grant: no publish scope → `formats: []` with the reason.
- Publish: `POST /{user}/threads` (TEXT | IMAGE | VIDEO | CAROUSEL with `children`) then `POST /{user}/threads_publish`; media containers are polled on `status` until `FINISHED`. Limits (docs, 2026-09-05): 500 characters, carousel 2–20, image ≤8 MB, video ≤1 GB and ≤300 s, **250 posts and 1,000 replies per profile per 24 h** (`estimatePublishCost` carries the post cap).
- Idempotency: none in the API. The adapter remembers each attempt in-process and `findPublication` matches the profile's recent posts by text at or after the attempt started; an attempt this process never saw is honestly `null`.
- Inbox: replies on the last 25 posts via `GET /{media}/conversation`; a reply is a TEXT container with `reply_to_id`; `findReply` is structural over `GET /me/replies`. Mentions arrive only by webhook (Advanced Access + verified business) → declared off with the reason. Insights: per-post `views,likes,replies,reposts,quotes,shares` (lifetime, on the fetch day; the last three fold into `shares`), daily `views` and `followers_count` at profile level.
- **Not supported, declared with reasons:** AI-disclosure field (caption line instead), cover frame, ads, webhooks until Advanced Access, first comment as a field, DMs, reviews. Uninstall / delete callbacks reuse Meta's `signed_request` parser with the Threads secret.

### Bluesky
- Product: the **AT Protocol** XRPC API on the account's service host (`bsky.social` by default; `BLUESKY_SERVICE` for a self-hosted PDS) plus `video.bsky.app` for video. No developer app exists; `PROVIDERS_ENABLE_BLUESKY=1` is the whole switch.
- **Sign-in is an app password, not OAuth.** The adapter declares `credentialsForm` (handle + app password) and `signIn`; `authorizationUrl`/`exchangeCode` throw. The platform renders the form at `/app/:ws/accounts/connect/bluesky`. A main password is refused by shape (`xxxx-xxxx-xxxx-xxxx`) before any network call. `createSession` → access JWT (expiry read from the token) + refresh JWT; `refreshSession` rotates both; `deleteSession` on disconnect.
- Publish: one `com.atproto.repo.createRecord` of `app.bsky.feed.post`. **One embed per post**: up to 4 images (`uploadBlob`, ≤1,000,000 bytes each), or one MP4 (service-auth token → `app.bsky.video.uploadVideo` → `getJobStatus` polled to `JOB_STATE_COMPLETED`, ≤300 MB), or a link card when the post has no media. URLs, `@handle.tld` mentions (resolved to DIDs) and `#tags` become facets at **UTF-8 byte offsets**.
- Text limit is **300 graphemes** (and 3,000 bytes): the adapter drops the generic UTF-16 length issue and counts with `Intl.Segmenter`.
- **Idempotent by construction:** the record key is a TID derived from the idempotency key (`tidFromKey`), so a retry can never create a second post and `findPublication`/`findReply` are a `getRecord`, not a scan.
- Inbox: `app.bsky.notification.listNotifications` filtered to `reply` (→ comment), `mention` and `quote` (→ mention); a reply is a post with `root`/`parent` strong refs read from the parent via `getPosts`. Insights: `getPosts` counts (likes → reactions, replies → comments, reposts + quotes → shares) and `getProfile.followersCount`; **Bluesky publishes no views or impressions.**
- Rate limits (docs): 5,000 write points/hour and 35,000/day per account (a create is 3), 30 `createSession` per 5 minutes; `ratelimit-reset` is an epoch and is mapped to `retryAfterSeconds`.
- **Not supported, declared with reasons:** DMs (chat-scoped app password + `chat.bsky` service; not wired), webhooks (the firehose is whole-network), ads, reviews, audience demographics, AI-disclosure field, cover frame, first comment as a field. AT Protocol OAuth (hosted `client_metadata.json`, DPoP) is deferred.
